...
if(slotSettings.Jackpots['jackPay2']!=undefined){
...
var cprop=libArr['lnWinSym_'+cSym];	
	...
	winArr[j][cPos].StartAnim({x:rPos[j-1].x-17,y:rPos[j-1].y+(symHeight*cPos)-20});		
}
...
var cprop=libArr['lnWinSym_'+cSym];
	...
	borderArr[j].x=winArr[j][cPos]._view.x+objectsSettings['lnBorder'+0].offset+17;
	borderArr[j].y=winArr[j][cPos]._view.y+objectsSettings['lnBorder'+0].offset+20;
	
	if(firstBorder){
	firstBorder=false; 
        borderMiniArr[j]=new MiniBorder(self._view,[objArr['lnBorderMin0_0'],objArr['lnBorderMin0_1'],objArr['lnBorderMin0_2']],slotSpinResult.bonusInfo.scattersWin,0,{x:winArr[j][cPos]._view.x+objectsSettings['lnBorderMin'+0].offsetX+17,y:winArr[j][cPos]._view.y+20+objectsSettings['lnBorderMin'+0].offsetY},objectsSettings['lnBorderMin'+0]);
	}
