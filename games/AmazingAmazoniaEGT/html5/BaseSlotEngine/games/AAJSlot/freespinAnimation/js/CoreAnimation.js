...
(function()
{
	...
	function createAndPositionLinesForScreen(translations, valueText) {
		...
		var OffsetX = this._defaultSettings.OffsetX;
		...
		for (var i = 0; i < lines.length; i++) {
			...
			shadowStyle = translations.settings.shadowStyle;

			// Some line text could have different font size, color, stroke and shadow
			...
			if (typeof(jsonItem.OffsetX) !== 'undefined') {
				OffsetX = jsonItem.OffsetX;
			}
			...
			if (textItem.indexOf("*") != -1) {
				...
				for (var k = 0; k < specialLine.length; k++) {
					...
					if (specialLine[k].indexOf("~") != -1) {
						...
						if (typeof(jsonItemNumber.OffsetX) !== 'undefined') {
							OffsetX = jsonItemNumber.OffsetX;
						}		

						if (typeof(jsonItemNumber.vGap) !== 'undefined') {
							vGap = jsonItemNumber.vGap;
						}				
				...
				this._textLinesArray.push({text: textItem, align: "center", font: setFont.call(this, fontSize), 
											fill: fillStyle, stroke: strokeStyle, shadow: shadowStyle, 
											x: self._context.canvas.clientWidth/2 + OffsetX, y: offsetY});
				OffsetX = 0;	
			}
